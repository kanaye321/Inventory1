...
export class MemStorage implements IStorage {
  ...
  async getBitlockerKeys(): Promise<any[]> {
    try {
      ...
      if (this.databaseStorage) {
        console.log('Attempting to fetch BitLocker keys from PostgreSQL...');
        return await this.databaseStorage.getBitlockerKeys();
      }
    } catch (error) {
      console.log('PostgreSQL not available for BitLocker keys, using memory storage fallback');
  ...
  async getBitlockerKey(id: number): Promise<any> {
    try {
      if (this.databaseStorage) {
        return await this.databaseStorage.getBitlockerKey(id);
      }
    } catch (error) {
      console.log('Database not available for BitLocker key, using memory storage');
    }
  ...
  async getBitlockerKeyBySerialNumber(serialNumber: string): Promise<any[]> {
    try {
      if (this.databaseStorage) {
        return await this.databaseStorage.getBitlockerKeyBySerialNumber(serialNumber);
      }
    } catch (error) {
      console.log('Database not available for BitLocker key search, using memory storage');
    }
  ...
  async getBitlockerKeyByIdentifier(identifier: string): Promise<any[]> {
    try {
      if (this.databaseStorage) {
        return await this.databaseStorage.getBitlockerKeyByIdentifier(identifier);
      }
    } catch (error) {
      console.log('Database not available for BitLocker key search, using memory storage');
    }
  ...
  async updateBitlockerKey(id: number, key: any): Promise<any> {
    try {
      if (this.databaseStorage) {
        return await this.databaseStorage.updateBitlockerKey(id, key);
      }
    } catch (error) {
      console.log('Database not available for BitLocker key update, using memory storage');
    }

    // Memory storage fallback
    const index = this.bitlockerKeys.findIndex(k => k.id === id);
    if (index === -1) return undefined;

    this.bitlockerKeys[index] = {
      ...this.bitlockerKeys[index],
      ...key,
      updatedAt: new Date().toISOString()
    };

    return this.bitlockerKeys[index];
  }

  async deleteBitlockerKey(id: number): Promise<boolean> {
    try {
      if (this.databaseStorage) {
        return await this.databaseStorage.deleteBitlockerKey(id);
      }
    } catch (error) {
      console.log('Database not available for BitLocker key deletion, using memory storage');
    }
    ...
    this.bitlockerKeys.splice(index, 1);
    return true;
  }
